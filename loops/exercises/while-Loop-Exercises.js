...
/*Exercise #4: Construct while loops to do the following:
  ...
  fuel_level = Number(input.question("Enter the starting fuel level : "));
 
  while ( fuel_level <= 5000 || fuel_level > 30000 || !Number.isInteger(fuel_level)){
    fuel_level = Number(input.question("Invalid input.Enter the starting fuel level : "));
     }
  
//b. Use a second loop to query the user for the number of astronauts (up to a maximum of 7). Validate the entry.
         
      numberOfAstronauts = Number(input.question("Enter the number of astronauts : "));
          
        while ( numberOfAstronauts <= 0 || numberOfAstronauts >= 7){
          numberOfAstronauts = Number(input.question("Invalid input. Enter the number of astronauts : "));
         }
